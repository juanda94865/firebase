...
function render() {
  ...
  // Teclado
  const keyboardDiv = createElement('div', {
    style: 'display:flex; flex-wrap:wrap; max-width: 400px; gap:6px; margin:10px 0;'
  });
...
async function guardarResultado(acierto) {
  ...
  const fecha = Date.now(); // evita problemas con caracteres no válidos en Firestore IDs
  ...
  try {
    const resultadosRef = doc(collection(db, 'resultados'), `${uid}_${fecha}`);
    await setDoc(resultadosRef, resultado);
...
async function fetchRandomPokemon() {
  ...
  try {
    ...
    app.innerHTML = '<p>Error cargando el Pokémon. Intenta otra vez.</p>';
  }
}
